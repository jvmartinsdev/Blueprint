...
mkad = [[1,37.842762, 55.774558], [2, 37.842789, 55.76522], [3,37.842627,55.755723], [4,37.841828,55.747399], [5,37.841217,55.739103], [6,37.840175,55.730482], [7,37.83916,55.721939],
...
#response = reqs.get('https://geocode-maps.yandex.ru/1.x/?apikey=411aa5d6-d222-4536-8d0b-707a7524d816&format=json&geocode=' + adress)
#position = response.metaDataProperty.GeocoderResponseMetaData.Points.pos .split(' ')
#lat = position[0]
#long = position[1]

#validate = coordValidate(lat, long)

#if(validate == False); 
#distance = distanceCalculator(lat, long)
#registerLog(distance, lat, long)

return 'Sucesso a distancia foi registrada no log'

def coordValidate(lat, long):
#Method that validate if exists lat and long in MKAD
retorno = False
for km in mkad: 
if lat == km[0] & long == km[1]:
retorno = True
return retorno
...
lat1 = radians(lat)
lon1 = radians(long)
lat2 = radians(55.7888)
lon2 = radians(37.6802)
...
def registerLog(distance, address):
logging.basicConfig(filename="logfilename.log", level=logging.INFO, filemode='a')
logging.info('Distancia entre o anel e '%address %' e: ' %distance %'m')
